# vbwd-android-invoice — architecture

Port of the iOS `invoice payment` plugin. Plugin id `invoice-payment`,
version `1.0.0`.
...
Invoice payment — the backend issues an invoice at checkout and emails it; no redirect.

Concretely it registers: translations only (registering no payment action makes checkout go straight to confirmation) — all via the `PlatformSdk` facade, never by
